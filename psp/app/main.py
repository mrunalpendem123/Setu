...
from .storage import TokenRecord, save_token, get_token
...
def _apply_idempotency(request: Request, body: bytes, payload: dict, status_code: int) -> JSONResponse:
    ...
    if existing:
        ...
        return JSONResponse(content=existing["payload"], status_code=existing["status_code"])

    _idempotency_store[key] = {"request_hash": req_hash, "payload": payload, "status_code": status_code}
    return JSONResponse(content=payload, status_code=status_code)
